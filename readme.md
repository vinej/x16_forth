...
**`SPREDIT.FTH`** is a sprite read / modify / create utility for the X16's VERA sprites (4bpp, 16 colours, colour 0 = transparent). It edits a sprite image kept in the KERNAL's designated sprite-image VRAM area (bank 1, `$3000`) and shows it live on hardware sprite 1 as you work. Word API: `16 16 SPNEW` makes a blank sprite (sizes 8/16/32/64), `c x y SPSET` / `x y SPGET` set/read a pixel, `c SPFILL` / `SPCLR` fill/clear, `SPDUMP` prints it as a grid of colour nibbles (`.` = transparent), `SPSHOW`/`SPHIDE` toggle the live preview, and `S" NAME" SPR-SAVE` / `SPR-LOAD` store the raw image bytes on device 8 (via `VSAVE`/`BVLOAD`). `INCLUDE SPREDIT.FTH` then `SPDEMO` draws a sample, or `SPED` opens the interactive editor: the arrow keys move a black-on-yellow cursor over the grid, `0`-`9` / `A`-`F` (either case) paint a colour - a swatch legend shows each - `SPACE` erases, and `Q` (or `q`) quits. It redraws incrementally (only the changed cells repaint) so it stays responsive; the on-screen grid works best for the 8/16/32 sizes (a 64-row grid is taller than the text screen).

**`SNDEDIT.FTH`** is a tune / song / noise editor - a monophonic step sequencer you can play on either the VERA PSG or the YM2151 FM synth, and save/reload so a game can `BLOAD` and play it. A song is a list of steps, each a note + a duration in jiffies; a note byte is `octave<<4 | semitone(1..12)`, `0` = rest (the encoding `PSGNOTE`/`FMNOTE` share; PSG waveform 3 = noise). Word API: `SG-CLR`, `note dur NOTE,` (append), `PSG` / `FM` (pick the synth), `n VOICE`, `SG-PLAY` (blocking, jiffy-timed), and `S" NAME" SG-SAVE` / `SG-LOAD` (raw bytes on device 8 - a game does `S" TUNE.SNG" SG-LOAD SG-PLAY`). `INCLUDE SNDEDIT.FTH` then `SG-DEMO SG-PLAY` for a sample, or `SNDED` for the interactive step editor: arrows move the cursor, the piano keys `ZSXDCVGBHNJM` set the pitch (`1`-`8` pick the octave), `+`/`-` change the note's duration, `INS`/`DEL` add/remove steps, `W` cycles PSG waveform (3 = noise), `I` cycles FM instrument, `T` toggles PSG/FM, `P` plays, `Q` quits. (Building it surfaced and fixed a real core bug: X16 `SLEEP` was reading the jiffy clock in C64 byte order and ignored its argument.)
