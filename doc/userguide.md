...
- **`VSYNC`** ( -- ) — pace to the video timing: spins until VERA's scanline counter wraps (about one frame). Use it to throttle a game loop and update VRAM without tearing. `BEGIN  move-things  draw-things  VSYNC  AGAIN` (call it twice for a slower loop). It reads VERA directly, so it never depends on interrupts being enabled.
